...
while current_known_number < all_measurements.shape[0]:
    ...
    lcm = GPy.util.multioutput.LCM(input_dim=1, num_outputs=2, kernels_list=[K_Matern, K_RBF])

    #lcm = GPy.util.multioutput.LCM(input_dim=1, num_outputs=3, kernels_list=[K_Matern, K_RBF])

    model = GPy.models.GPCoregionalizedRegression(X_list=[X_days, X_days],
    ...
    # For LCM
    model['.*W'].constrain_fixed(0)

    #model['.*W'].constrain_fixed(0)
    #model['.*lengthscale'].constrain_fixed(1.5)
    #model['.*periodic_exponential.period'].constrain_fixed(7)
    ...
    newX = np.hstack([newX, np.zeros_like(newX)])

    noise_dict = {'output_index':newX[:, 1].astype(int)}

    Task_main_predicted, Task_main_vs = model.predict(Xnew=newX, Y_metadata=noise_dict)
