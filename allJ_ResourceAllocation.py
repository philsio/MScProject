...
test_set_full_Y = patients_per_day[STARTING_DAYS:]

true_mean = np.mean(test_set_full_Y)
true_std = np.std(test_set_full_Y)
true_beds = true_mean + 1.96 * true_std
total_true_beds = true_beds * test_set_full_Y.shape[0]

model_beds = np.sum(uc_bound)

print("Number of beds predicted by basic approach: ", total_true_beds)
print("Number of beds predicted using model UC bounds: ", model_beds)
