...
while current_known_number < lagged_dataset_X.shape[0]:
    ...
    K_RBF_1 = GPy.kern.RBF(X_train_all.shape[1])
    K_RBF_2 = GPy.kern.RBF(X_train_all.shape[1])

    K1 = K_RBF_1
    K2 = K_RBF_2

    model = GPy.models.GPRegression(X_train_all,
                                    Task_main,
                                    kernel=K1)
    model_extra = GPy.models.GPRegression(X_train_all,
                                          Task_PM25,
                                          kernel=K2)

    model.optimize_restarts(num_restarts=5)
    model_extra.optimize_restarts(num_restarts=5)
    print(model)
    ...
    for day in range(0, Task_main_test.shape[0]):
        ...
        newX = np.expand_dims(next_predict_X, axis=1)
        ...
        # Using the single entry next_predict_X, predict one day ahead
        newX = np.expand_dims(next_predict_X, axis=1)
        current_prediction_extra, current_v_extra = model_extra.predict(Xnew=newX.T)
        ...
        if day < Task_main_test.shape[0]-1:
            ...
            for next_day in range(0, next_end):
                ...
                # ...most recent PM2.5 prediction goes at the back of the list
